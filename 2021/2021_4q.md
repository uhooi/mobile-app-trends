...
- 参考: [FY2021 3Q](./fy2022_1Q.md#アカウント削除機能の追加期限が発表-(10/06))
...
- `swift package` コマンドがコマンドプラグインをサポートした
  ...
  - パッケージ上でカスタムアクションを実行するために、コマンドが定義できるようになった
    - 例 1: DocCでドキュメントを生成する
    - 例 2: swift-formatでソースコードを整形する
    - 例 3: 配布用のアーカイブを生成する
...
- Swift Developers Japan の Discord アーカイブが公開された
